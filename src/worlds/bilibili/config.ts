...
export const BILIBILI_CONFIG_GROUP: ConfigGroup = {
  ...
  schema: {
    ...
    properties: {
      ...
      'worlds.bilibili.overlay.tts.enabled': {
        type: 'boolean',
        title: '公告语音播报',
        'x-hot': false,
        description: 'Agent 公告更新时经本机 TTS 接口合成语音,在 Overlay 页面播放。改动需重启后生效。',
      },
      'worlds.bilibili.overlay.tts.apiUrl': {
        type: 'string',
        title: 'TTS 接口',
        'x-hot': false,
        description: '合成接口地址,如 GPT-SoVITS api_v2 的 http://127.0.0.1:9880/tts。',
      },
      'worlds.bilibili.overlay.tts.params': {
        type: 'string',
        title: 'TTS 参数',
        'x-hot': false,
        description: '以 & 连接的 query 参数,随 text 一起转发给 TTS 接口(声线参考音频、提示词等)。',
      },
      'worlds.bilibili.overlay.tts.timeoutMs': {
        type: 'integer',
        title: '合成超时',
        minimum: 1000,
        maximum: 120000,
        ...
        'x-suffix': 's',
        'x-hot': false,
        description: '单次合成等待上限;超时该条语音被跳过。',
      },
      'worlds.bilibili.overlay.tts.maxChars': {
        type: 'integer',
        title: '单次最多朗读',
        minimum: 1,
        maximum: 2000,
